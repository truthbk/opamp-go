...
func TestServerToAgentCommand(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			...
			receiver := NewWSReceiver(TestLogger{t}, callbacks, nil, &sender, &clientSyncedState, nil, new(sync.Mutex), time.Second, nil)
...
func TestServerToAgentCommandExclusive(t *testing.T) {
	...
	for _, test := range cases {
		...
		receiver := NewWSReceiver(TestLogger{t}, callbacks, nil, nil, &clientSyncedState, nil, new(sync.Mutex), time.Second, nil)
		receiver.processor.ProcessReceivedMessage(context.Background(), &protobufs.ServerToAgent{
			Command: &protobufs.ServerToAgentCommand{
				Type: protobufs.CommandType_CommandType_Restart,
			},
...
func TestReceiverLoopStop(t *testing.T) {
	...
	capabilities := protobufs.AgentCapabilities_AgentCapabilities_AcceptsRestartCommand
	clientSyncedState.SetCapabilities(&capabilities)
	receiver := NewWSReceiver(TestLogger{t}, callbacks, conn, &sender, &clientSyncedState, nil, new(sync.Mutex), time.Second, nil)
...
func TestWSPackageUpdatesInParallel(t *testing.T) {
	...
	clientSyncedState.SetCapabilities(&capabilities)
	receiver := NewWSReceiver(&internal.NopLogger{}, callbacks, nil, sender, clientSyncedState, localPackageState, &mux, time.Second, nil)
...
func TestRecieveMessage(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			rec := NewWSReceiver(&internal.NopLogger{}, callbacks, conn, NewSender(&internal.NopLogger{}), state, nil, new(sync.Mutex), time.Second, nil)
